    ...
    <div class="col-lg-12">
        <div class="ibox float-e-margins">
            ...
            <div class="ibox-content">
                <x-backend.filter
                    createRoute="visit_request.create"
                    submitRoute="visit_request.index" />
                @php
                $columns = [
                'customer' => ['label' => 'Khách hàng', 'render' => fn($item) => '<b>' . e($item->full_name) . '</b><br><small>' . e($item->phone) . ' - ' . e($item->email) . '</small>'],
                'property' => ['label' => 'Bất động sản', 'render' => fn($item) => $item->properties?->title ?? 'N/A'],
                'appointment' => ['label' => 'Ngày hẹn', 'render' => fn($item) => $item->appointment?->format('H:i d-m-Y') ?? 'N/A'],
                'status' => ['label' => 'Trang thái', 'render' => fn($item) => match($item->status) {
